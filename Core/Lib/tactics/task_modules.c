...
volatile target plant_target_2 = { 0, 0 };

int8_t task_go_home(target home, uint8_t direction) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			if (direction == WALL)
...
int8_t task_pickup_plants(target plant_target, float offset_perc) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			sensors_case_timer = SENSORS_MECHANISM;
	...
	case 2:
		...
		move_to_xy_offset(plant_target_2.x, plant_target_2.y, MECHANISM,
				110 * offset_perc);
		task_status = TASK_IN_PROGRESS;
		if (timer_delay_nonblocking(1432)) {
			task_case++;
			task_init = false;
		}
		if (interrupted) {
	...
	case 3:
		transition_factor = 1.0;
		mechanism_half_up();
		if (timer_delay_nonblocking(640)) {
			task_case++;
			task_init = false;
		}
		if (interrupted) {
...
int8_t task_dropoff_x_alt(uint8_t side, uint8_t planter) {
	switch (task_case) {
	case 0: {
		sensors_case_timer = SENSORS_HIGH;
		task_init = true;
		...
		if (planter == CLOSE) {
			if (side == BLUE)
				turn_to_pos(planter_blue_x_close.x + 750,
						planter_blue_x_close.y,
						WALL);
			else
			...
			if (side == BLUE)
				...
				turn_to_pos(planter_yellow_x_far.x + 750,
						planter_yellow_x_far.y,
						WALL);
		}
		if (movement_finished() && timer_delay_nonblocking(20)) {
			task_case++;
...
int8_t task_dropoff_x(uint8_t side, uint8_t planter) {
	switch (task_case) {
	case 0: {
		sensors_case_timer = SENSORS_HIGH;
		task_init = true;
	...
	case 2:
		...
		if (planter == CLOSE) {
			...
			if (side == BLUE)
				move_to_xy(planter_blue_x_far.x - 200,
						planter_blue_x_far.y - 150,
						WALL);
			else
...
int8_t task_dropoff_y_2(uint8_t side, uint8_t first_side) {
	switch (task_case) {
	case 0: {
		if (first_side == WALL)
			sensors_case_timer = SENSORS_HIGH;
...
int8_t task_solar(uint8_t side, uint8_t solar, float speed_limit,
		uint8_t first_side) {
	switch (task_case) {
	case 0:
		if (!task_init) {
		...
		mechanism_up();
		turn_to_pos(side * 3000 - (2 * side - 1) * (320 + solar * 1000), 215,
				first_side);
		if (movement_finished() && timer_delay_nonblocking(20)) {
			task_case++;
			task_init = false;
		...
		break;
	case 1:
		move_to_xy(side * 3000 - (2 * side - 1) * (320 + solar * 1000), 215,
				first_side);
		if (movement_finished() && timer_delay_nonblocking(20)) {
			task_case++;
			task_init = false;
	...
	case 5:
		...
		move_to_xy(side * 3000 - (2 * side - 1) * (775 + solar * 1000), 215,
		MECHANISM);
		if (movement_finished() && timer_delay_nonblocking(20)) {
			task_case++;
		}
...
int8_t task_pot_reserved(uint8_t side) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			sensors_case_timer = SENSORS_MECHANISM;
...
int8_t task_pot_solar(uint8_t side) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			sensors_case_timer = SENSORS_MECHANISM;
...
int8_t task_pot_far(uint8_t side) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			sensors_case_timer = SENSORS_MECHANISM;
...
int8_t task_push_pots(uint8_t side) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			sensors_case_timer = SENSORS_MECHANISM;
...
int8_t task_push_pots_far(uint8_t side) {
	switch (task_case) {
	case 0:
		if (!task_init) {
			sensors_case_timer = SENSORS_MECHANISM;
...
void set_task_case(uint8_t number) {
	task_init = false;
	task_case = number;
}

void just_set_task_case(uint8_t number) {
	task_case = number;
}

uint8_t get_and_reset_task_points() {
